...
class SENet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, include_top=True):
        ...
        self.ld = nn.Linear(512 * block.expansion, 128)
        if include_top:
    ...
    def forward(self, anchor, positive, negative):
        anchor_feature = self.forward_once(anchor)
        positive_feature = self.forward_once(positive)
        negative_feature = self.forward_once(negative)
